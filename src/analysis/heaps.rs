...
fn binary_heap_vec_measurements() {
    ...
    for n in [10_000, 50_000,  100_000, 150_000, 200_000, 250_000, 300_000, 350_000, 400_000, 450_000, 500_000, 700_000, 1_000_000, 2_000_000, 4_000_000] {
        let d = measure_execution(|| {
            heap::insert_n_elements(&mut heap, n);
        });
        heap.clear();
...
pub fn register_commands(cm: &mut CommandMap) {
    cm.add(CommandDescriptor::new("analyze-heap-leftist", "Run runtime analysis for leftist heap", analyze_heap_leftist));
    cm.add(CommandDescriptor::new("analyze-heap-binomial", "Run runtime analysis for binomial heap", analyze_heap_binomial));
    cm.add(CommandDescriptor::new("analyze-heap-binary", "Run runtime analysis for binary heap", analyze_heap_binary));
}